...
function Labtable() {
  const [LabExams] = useState([
    { patientName: 'John Doe', type: 'Testosteron', appointmentDate: '04/05/2024', account: '2550' }
  ]);

  return (
    <div className='bg-white px-5 pb-5 shadow-xl rounded-lg mt-5'>
      ...
      <table className="min-w-full">
        ...
        <tbody>
          {LabExams.map((exam, index) => (
            <tr key={index} className='text-[11px] font-bold bg-[#ddf4fc] h-[34px] '>
              <td>{exam.patientName}</td>
              <td>{exam.type}</td>
              <td>{exam.appointmentDate}</td>
              <td>{exam.account}</td>
              <td>
                <a href=""><button className="text-center pl-2 text-lg hover:text-[#00afee] "><FaEye /></button></a>
              </td>
            </tr>
          ))}
        </tbody>
